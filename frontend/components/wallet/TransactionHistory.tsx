...
export default function TransactionHistory({
  ...
  if (!transactions.length) {
    return (
      <div className="rounded-3xl border border-zinc-800 bg-[#111827] p-12">
        <div className="flex flex-col items-center justify-center">
          <div className="rounded-full bg-orange-500/20 p-6">
            <Wallet className="h-12 w-12 text-orange-400" />
          </div>
          <h2 className="mt-6 text-2xl font-bold text-white">No Transactions Yet</h2>
    ...
    <div className="rounded-3xl border border-zinc-800 bg-[#111827] p-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500">
              Showing <span className="font-medium text-white">{visibleTransactions.length}</span> of{" "}
              <span className="font-medium text-white">{annotatedTransactions.length}</span> transaction
      ...
      {hasMore && (
        <button
          ...
          className="mt-8 flex w-full items-center justify-center gap-2 rounded-xl border border-zinc-800 py-3 text-sm font-medium text-zinc-400 transition-all duration-200 hover:border-orange-500 hover:bg-orange-500/5 hover:text-orange-400"
        >
          Load More
